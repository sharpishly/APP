...
class SyncapplyModel extends Model {
    ...
    public function index($data,$models,$options){
		...
		if($this->directive($data,__FUNCTION__)){
			...
			$fields = array(
				'Whats the catch?'=>'syncapply/index',
			...
			$h2 = 'Sick and tired of completing numerous forms when applying a job, 
			degree, housing, banking, tv, mobile, streaming service or even the gym? So are we! We are so sick of it that our company
			manifesto is sign up just once and <b>apply forever</b>';
	...
	public function slideshow_source($attr,$i){
		...
		$arr = array(
			'srcset'=>"/syncapply/images/toy_0" . $i . ".webp",
			'type'=>"image/webp"
		);
