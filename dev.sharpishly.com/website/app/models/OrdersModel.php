...
class OrdersModel extends Model {
    ...
    public function main($data,$models){
		...
		$data = $this->set($data,'tbl','migrate_products_orders');
	...
	public function recordjoins($data,$conditions,$options){
		...
		$conditions['joins']=array(						
			array(
				...
				'table'=>'migrate_users',
				'fields'=>array(
					'last_name'=>'biz_last_name',
					'first_name'=>'biz_first_name',
					// 'title'=>'biz_title',
    ...
    public function index($data,$models,$options){
		...
		if($this->directive($data,__FUNCTION__)){
			...
			if($debug){
				...
				$conditions = [
					'table' => 'migrate_users', // Main table
					'joins_many' => [
						[
							'type' => 'INNER',
							'table' => 'migrate_products_orders',
							'on' => [
						...
						// [
						// 	'type' => 'LEFT',
						// 	'table' => 'migrate_products',
						// 	'on' => [
						// 		'product_id' => 'id', // carts.id = orders.cart_id (This assumes cart_id in orders refers to carts.id)
						// 	]
						// ]
					],
					'fields' => [
