...
class SeedModel extends Model {
    ...
    public function main($data,$models){
        ...
        $options = array('function'=>__FUNCTION__);

		$data = $this->set($data,'tbl','migrate_seeds');
	...
	public function seeding($data,$title,$decription,$seed){
		...
		if(!isset($rs['result'][0]['id'])){
			...
			new dBug($data[__FUNCTION__] = $err);
		}

		return $data;
	}

	public function set_conditions($data,$fields, $options){
	...
	public function admin($data,$models,$options){
		...
		$fields = array(
			array(
				'title' => 'Headers',
				'description' => 'Add keywords to header tag',
				'url'=>'headers/index',
			),
			array(
			...
			array(
				'title' => 'Vacancies',
				'description' => '/vacancies/index',
				'url'=>'vacancies/index/',
			),
			array(
				'title' => 'Migrations',
				'description' => 'Create new tables',
				'url'=>'migrations/index',
			)
		);
	...
	public function products($data,$models,$options){

		$options['tbl'] = 'migrate_products';
		...
		$data = $this->seeding($data,'products_test_4','foo',$conditions);

		return $data;
	}

	public function business($data,$models,$options){

		$options['tbl'] = $data['tbl_businesses'];
		...
		$data = $this->seeding($data,'chorlton_shops_test_4','foo',$conditions);
	...
	public function seed($data,$models,$options){
		...
		if($this->directive($data,__FUNCTION__)){
			...
			$data = $this->products($data,$models,$options);

			$data = $this->business($data,$models,$options);
