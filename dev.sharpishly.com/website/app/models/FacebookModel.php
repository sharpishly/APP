...
class FacebookModel extends Model {
    ...
    public function main($data,$models){
		...
		$data = $this->profile($data,$models,$options);
	...
	public function profile($data,$models,$options){
		
		if($this->directive($data,__FUNCTION__)){

			$data = $this->set($data,'facebook_sdk',$this->request());

			$title = ucfirst(__FUNCTION__);

			$data = $this->if_user_exists($data);

			$data = $this->get_new_user_tokens($data);
            ...
            $data = $this->partials->template($data, 'h2', 'Info here');
	...
	public function refresh_token($data){
		...
		if(isset($data['get_new_user_tokens']['result'][0][__FUNCTION__])){

			new dBug($rs = $data['get_new_user_tokens']['result'][0][__FUNCTION__]);
	...
	public function if_user_exists($data,$model=false,$options=false){
		...
		$wheres = array(
			'email'=>$this->session->getFacebook('email')
		);
	...
	public function save_new_user($data,$model=false,$options=false){
		...
		if(!isset($data['if_user_exists']['result'][0])){
			...
			$save = array(
				'first_name'=>$this->session->getFacebook('first_name'),
				'last_name'=>$this->session->getFacebook('last_name'),
				'email'=>$this->session->getFacebook('email'),
	...
	public function save_new_user_tokens($data,$model=false,$options=false){
		...
		if(isset($data['if_user_exists']['result'][0])){
			...
			$save = array(
				'access_token'=>$this->session->getFacebook('access_token'),
				'refresh_token'=>$this->session->getFacebook('refresh_token'),
				'user_id'=>$user_id,
				'expiry_time'=>$date,
				'created_at'=>$date,
				'updated_at'=>$date,
	...
	public function auth($data,$models,$options){
		...
		if($this->directive($data,__FUNCTION__)){
			...
			$data = $this->save_new_user_tokens($data);
            ...
            $data = $this->partials->template($data, 'h2', '');
