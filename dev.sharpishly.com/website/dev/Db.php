...
class Db {
    ...
    public function find($conditions) {
        ...
        // Build WHERE clause string and store in $data['wheres']
        $data = $this->where($data, $conditions);
        $data = $this->wheres($data, $conditions);
        $data = $this->like($data, $conditions);

        // Build JOIN clause string and store in $data['joins']
        $data = $this->joins($data, $conditions);
        $data = $this->joins_many($data, $conditions);

        // Generate subqueries (they use $data['wheres'])
        $data = $this->find_sum($data, $conditions);
        $data = $this->subquery_count($data, $conditions);
        $data = $this->find_grand_total($data, $conditions);

        // Describe statement now builds the full SQL including SELECT, FROM, JOINs, and WHERE
    ...
    public function joinsget($data, $conditions) {
        ...
        // Append to the $data['joins'] string, not $data['sql'] directly
        if (!isset($data[$k])) {
            $data[$k] = '';
        }
        $data[$k] .= ' ' . $data[$e]['type'];
    ...
    public function joins($data, $conditions) {
        if (isset($conditions[__FUNCTION__])) {
            // Initialize $data['joins'] if not already set by joins_many
            if (!isset($data[__FUNCTION__])) {
                $data[__FUNCTION__] = '';
            }
    ...
    public function joins_many($data, $conditions) {
        if (isset($conditions[__FUNCTION__])) {
            // Initialize if not already set by 'joins'
            if (!isset($data['joins'])) {
                $data['joins'] = '';
            }
    ...
    public function like($data, $conditions) {
        if (isset($conditions['like'])) {
            $like_clause = " " . $conditions['table'] . ".`" . $conditions['like']['col'] . "` LIKE '%" . $conditions['like']['val'] . "%'";
            
            // This now adds to the main 'wheres' string. 'wheres' method will combine.
            if (!isset($data['wheres_parts'])) {
                $data['wheres_parts'] = [];
            }
            $data['wheres_parts'][] = $like_clause;
        }
        return $data;
    }
    ...
    public function or_and($data, $conditions) {
        if (isset($conditions['wheres']['or'])) {
            ...
            foreach ($it as $el) {
                ...
                $inner_or_parts = [];
                foreach ($rs as $val) {
                    $inner_or_parts[] = $col . " = '" . $val . "'"; 
                }
                $or_clause_parts[] = "(" . implode(" OR ", $inner_or_parts) . ")";
            }
            $conditions['or_clause_generated'] = implode(" OR ", $or_clause_parts);
        }
        return $conditions;
    }
    ...
    public function wheres($data, $conditions) {
        $e = __FUNCTION__;
        $final_where_parts = $data['wheres_parts'] ?? []; // Start with parts from 'like' or 'where'

        $conditions = $this->or_and($data, $conditions); // Process 'or' for later combination

        // Handle simple 'where' conditions from $conditions['where']
        if (isset($conditions['where'])) {
            $col = $conditions['where']['col'];
            $val = $conditions['where']['val'];
            $final_where_parts[] = $conditions['table'] . ".`" . $col . "` = '" . $val . "'";
        }

        // Handle 'wheres' array conditions (excluding 'or' which is processed separately)
        if (isset($conditions[$e])) {
            $it = $conditions[$e];
            if (isset($it['or'])) {
                unset($it['or']); // Avoid double processing of 'or' conditions
            }
            $it = new \ArrayIterator($it);
            $it = new \CachingIterator($it);
            $tbl = $conditions['table'];
            foreach ($it as $el) {
                $final_where_parts[] = $tbl . ".`" . $it->key() . "` = '" . $it->current() . "'";
            }
        }

        $where_string = '';
        if (!empty($final_where_parts)) {
            $where_string .= " WHERE " . implode(" AND ", $final_where_parts);
        }

        if (isset($conditions['or_clause_generated']) && !empty($conditions['or_clause_generated'])) {
            if (empty($where_string)) {
                $where_string .= " WHERE ";
            } else {
                $where_string .= " AND ";
            }
            $where_string .= "(" . $conditions['or_clause_generated'] . ")";
        }

        $data[__FUNCTION__] = $where_string; // Store the complete WHERE clause string
        return $data;
    }

    public function where($data, $conditions) {
        // This method now adds to the temporary 'wheres_parts' array
        // It's less used if 'wheres' is the primary builder, but kept for compatibility.
        if (isset($conditions['where'])) {
            if (!isset($data['wheres_parts'])) {
                $data['wheres_parts'] = [];
            }
            $data['wheres_parts'][] = $conditions['table'] . ".`" . $conditions['where']['col'] . "` = '" . $conditions['where']['val'] . "'";
        }
        return $data;
    }
    ...
    public function describe_total_statement($conditions, $data) {
        if (isset($data['fields'])) {
            ...
            // Append JOINs and WHERE clause if available
            if (isset($data['joins']) && !empty($data['joins'])) {
                $sql .= " " . $data['joins'];
            }
            if (isset($data['wheres']) && !empty($data['wheres'])) {
                $sql .= " " . $data['wheres'];
            }
            $data['sql'] = $sql;
        }
    ...
    public function describe_statement($conditions, $data) {
        if (isset($data['fields'])) {
            ...
            // Append the grand total, sum, and subquery_count after all other fields
            ...
            // --- IMPORTANT: Append JOINs and WHERE clause here! ---
            if (isset($data['joins']) && !empty($data['joins'])) {
                $sql .= " " . $data['joins'];
            }
            if (isset($data['wheres']) && !empty($data['wheres'])) {
